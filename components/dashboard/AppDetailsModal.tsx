...
export function AppDetailsModal({
  ...
  useEffect(() => {
    async function fetchAppDetails() {
      ...
      try {
        ...
        setIsGooglePlayAdded(
          appResponse.data?.status === "testers_added_google_play"
        );
      } catch (error) {
  ...
  const handleGooglePlayStatusChange = async (checked: boolean) => {
    ...
    try {
      ...
      toast.success(
        `App status updated to ${
          checked ? "Testers Added to Google Play" : "Testers Added"
        }`
      );
    } catch (error) {
          ...
          >
                ...
                {app.status === "test_started" && (
                  <div className="bg-indigo-50 p-3 rounded-md border border-indigo-100">
                    ...
                    <p className="text-xs text-indigo-700 mt-1">
                      The testing phase has been started for this app. You'll
                      receive updates as the testing progresses.
                    </p>
                  </div>
                )}

                {/* Google Play Status Checkbox - Only visible for admins when status is testers_added */}
                {isAdmin && app.status === "testers_added" && (
            ...
            <Card>
              ...
              <CardContent>
                {purchases.length === 0 ? (
                  <p className="text-sm text-muted-foreground">
                    No purchases found for this app.
                  </p>
                ) : (
