...
export function UserDetailsModal({ userId, open, onOpenChange, onTestDetailsAdded }: UserDetailsModalProps) {
	...
	const handleTestDetailsSubmitted = async () => {
		...
		if (userId) {
			const { data } = await supabase.from("apps").select("*").eq("user_id", userId);
			setApps(data || []);
		}
	...
	const handleSubmitTesters = async () => {
		...
		try {
			...
			// Refresh apps data
			const { data: appsData } = await supabase.from("apps").select("*").eq("user_id", userId);
	...
	const handleTestStartedChange = async (appId: string, checked: boolean) => {
		...
		try {
			...
			// Update local state
			setApps((prevApps) => prevApps.map((app) => (app.id === appId ? { ...app, status: "test_started" } : app)));
		} catch (error) {
											...
											<CardContent className='pt-6'>
												<div className='flex justify-between items-start'>
													<div>
														...
														{app.app_review && (
															<p className='text-sm mt-2'>
																<span className='font-medium'>Review:</span> {app.app_review}
															</p>
														)}
														{app.app_screenshots && app.app_screenshots.length > 0 && (
															<p className='text-sm'>
																<span className='font-medium'>Screenshots:</span> {app.app_screenshots.length} images
															</p>
														)}
														{app.tester_accounts && (
															<p className='text-sm mt-2'>
																<span className='font-medium'>Tester Accounts:</span>{" "}
													...
													<div className='space-x-2'>
														<Button size='sm' variant='outline' onClick={() => handleAddTestDetails(app.id)}>
															...
															app.status !== "test_started" ? (
																<div className='inline-flex items-center space-x-2 mt-2'>
																	<Checkbox
																		...
																		onCheckedChange={(checked) => handleTestStartedChange(app.id, checked === true)}
																	...
																	{updatingStatusAppId === app.id && <Loader2 className='h-4 w-4 animate-spin ml-2' />}
																</div>
															) : null
														) : (
															<Button size='sm' variant='outline' onClick={() => handleAddTesters(app.id)}>
							...
							{showTesterForm && selectedAppId && (
								<div className='border rounded-lg p-4 space-y-4'>
									...
									<div className='flex justify-end space-x-2'>
										<Button variant='outline' onClick={() => setShowTesterForm(false)}>
											Cancel
										</Button>
										<Button onClick={handleSubmitTesters} disabled={isSubmittingTesters}>
											{isSubmittingTesters && <Loader2 className='mr-2 h-4 w-4 animate-spin' />}
