...
export function TestDetailsModal({ appId, open, onOpenChange, onSubmitted }: TestDetailsModalProps) {
	...
	const handleSubmit = async () => {
		...
		try {
			...
			toast.error(error instanceof Error ? error.message : "Failed to update test details");
		} finally {
						...
						<div className='border border-input rounded-md p-4'>
							<div className='flex flex-wrap gap-2 mb-3'>
								{uploadedFiles.map((file, index) => (
									<div key={index} className='relative group border rounded-md overflow-hidden h-20 w-20'>
										<img
